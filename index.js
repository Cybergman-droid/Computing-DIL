document.getElementById("numbersort").addEventListener("click", numberProgram);
...
class Spaceship {
  ...
  setShieldStrength(inShieldStrength) {
    inShieldStrength >= 0 && inShieldStrength <= 100
      ? (this.#shieldStrength = inShieldStrength)
      : console.alert("enter a vaild sheid strength");
  }
}
...
class Cell {
  ...
  consumeEnergy(amount) {
    amount > this.#energy
      ? console.warn("Energy not available")
      : (this.#energy -= amount);
  }
